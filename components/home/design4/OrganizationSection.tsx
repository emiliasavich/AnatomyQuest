      ...
      <ul className="mt-6 grid gap-5 sm:grid-cols-3 sm:items-stretch">
        {MAIN_SECTIONS.map((item) => (
          <li key={item.href} className="flex min-h-0">
            <Link
              href={item.href}
              className="flex h-full flex-col rounded-lg border border-stone-200 bg-white p-5 transition hover:border-aq-blue/50 hover:bg-stone-50/80"
            >
              <span className="font-display font-semibold text-aq-blue">{item.label}</span>
              <span className="mt-2 flex-1 text-sm text-stone-600">{item.description}</span>
              <span className="mt-4 text-sm font-medium text-aq-blue">Explore →</span>
            </Link>
          </li>
        ))}
      </ul>
