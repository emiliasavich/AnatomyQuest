  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
        {SITE_MAP.map((block) => (
          <div key={block.section} className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm">
            <h3 className="font-display font-semibold text-stone-900">{block.section}</h3>
            <ul className="mt-3 space-y-2">
              {block.links.map((link) => (
                <li key={link.href}>
                  <Link href={link.href} className="text-sm text-aq-red hover:underline">
                    {link.label}
                  </Link>
                </li>
              ))}
