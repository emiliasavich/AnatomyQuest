...
function renderDescription(desc: unknown): React.ReactNode {
  ...
  if (desc && typeof desc === "object" && "has_subpoints" in desc) {
    ...
    return (
      <>
        {isHtml(d.has_subpoints.main_point) ? renderHtml(d.has_subpoints.main_point) : d.has_subpoints.main_point}
...
const STEP_STYLES = {
  location: { bg: "bg-[#2563eb]/10", border: "border-[#2563eb]/25", heading: "text-[#2563eb]" },
  shape: { bg: "bg-[#059669]/10", border: "border-[#059669]/25", heading: "text-[#059669]" },
  neighbors: { bg: "bg-[#7c3aed]/10", border: "border-[#7c3aed]/25", heading: "text-[#7c3aed]" },
  landmarks: { bg: "bg-[#dc2626]/10", border: "border-[#dc2626]/25", heading: "text-[#dc2626]" },
  blood: { bg: "bg-[#ea580c]/10", border: "border-[#ea580c]/25", heading: "text-[#ea580c]" },
} as const;
    ...
    <div className="relative space-y-8">
      ...
      <div className="rounded-2xl border border-stone-200/90 bg-stone-50/80 px-6 py-6 sm:px-8 sm:py-7">
        <p className="text-stone-700">
          ...
          Understand how the humerus relates to the body as a whole.
        </p>
        <p className="mt-3 text-stone-700">
          <span className="font-semibold text-stone-900">Purpose:</span>{" "}
          To encourage deeper, longer-lasting learning by linking the bone to the skeletal, muscular, and cardiovascular systems rather than relying on rote memorization.
        </p>
        <p className="mt-3 text-stone-700">
          <span className="font-semibold text-stone-900">How to use this guide:</span>{" "}
          Follow the{" "}
          <Link href="/getting_started/effective_learning_methods/learning_bones" className="font-medium text-aq-primary hover:underline">five structured steps</Link>
          {" "}below.
        </p>
        <AccordionSection id="five-steps-overview" title="Click to read about the five steps. Then click to close." defaultOpen={false}>
          <p className="mb-4 text-stone-700">Each step relates the bone to the bigger picture to help you understand the material and remember it for longer.</p>
          <div className="space-y-4 text-sm text-stone-700">
            <div>
              <p className="font-medium text-stone-900 location-color">Location</p>
              <p><span className="font-medium">Question:</span> Where is the bone?</p>
              <p><span className="font-medium">Purpose:</span> Helps you build a mental map of the skeleton.</p>
            </div>
            <div>
              <p className="font-medium text-stone-900 shape-color">Shape</p>
              <p><span className="font-medium">Question:</span> What is the bone&apos;s shape?</p>
              <p><span className="font-medium">Purpose:</span> Gives insight into how the bone interacts with the body.</p>
            </div>
            <div>
              <p className="font-medium text-stone-900 neighbors-color">Neighbors</p>
              <p><span className="font-medium">Question:</span> What bones articulate (form joints with) this bone? What is the type of each joint?</p>
              <p><span className="font-medium">Purpose:</span> Lets you examine what joints the bone is part of and what movements those joints allow.</p>
            </div>
            <div>
              <p className="font-medium text-stone-900 landmarks-color">Anatomical Landmarks</p>
              <p><span className="font-medium">Question:</span> What are the named parts of the bone? What is the significance of each?</p>
              <p><span className="font-medium">Purpose:</span> Connects each named location to the muscles, nerves, blood vessels, and other bones that interact with the location.</p>
            </div>
            <div>
              <p className="font-medium text-stone-900 blood-supply-color">Blood Supply</p>
              <p><span className="font-medium">Question:</span> What blood vessels nourish the bone?</p>
              <p><span className="font-medium">Purpose:</span> Relates the bone to the cardiovascular system and builds your mental map of the body&apos;s blood vessels.</p>
            </div>
          </div>
        </AccordionSection>
      </div>
      ...
      <section className={`rounded-2xl border px-6 py-6 sm:px-8 sm:py-7 ${STEP_STYLES.neighbors.bg} ${STEP_STYLES.neighbors.border}`}>
        ...
        {Array.isArray(neighbors) &&
          neighbors.map((bigPicture: Record<string, unknown>, idx: number) => {
            const elements = (bigPicture.elements as Record<string, unknown>[]) ?? [];
            const imageName = bigPicture.image_name != null ? String(bigPicture.image_name) : null;
            return (
              <div key={idx} id={`neighbors-${slugify(String(bigPicture.big_picture_name))}`} className="mt-8 first:mt-6">
                <h3 className="font-serif text-lg font-semibold tracking-tight text-stone-900">{String(bigPicture.big_picture_name)}</h3>
                {/* One row: overview image (labeled neighbors: proximal/distal - anterior - not isolated/isolated) */}
                <div className="mt-3 grid gap-6 sm:grid-cols-[auto_1fr] sm:items-start">
                  ...
                  <div className="min-w-0">
                    <p className="mb-3 text-stone-700">
                      The{" "}
                      <span className="popup-term" data-popup-id={`popup-${slugify(String(bigPicture.big_picture_name))}`}>
                        {String(bigPicture.big_picture_name).toLowerCase()}
                      </span>{" "}
                      end of the humerus{" "}
                      <span className="popup-term" data-popup-id="popup-articulate">articulates</span>
                      {" "}with the following bones:
                    </p>
                    <ul className="list-disc pl-6 mb-2">
                      {elements.map((el: Record<string, unknown>, i: number) => {
                        ...
                        return (
                          <li key={i}>
                            <button
                              type="button"
                              onClick={() => openAndScrollTo(accordionId, targetId)}
                    ...
                    <p className="mb-1 text-xs text-stone-500">Click the bulleted names for more information.</p>
                    <p className="text-xs text-stone-500">Click image to show/hide labels. Right click to enlarge.</p>
                  </div>
                </div>
                {/* Expandable detail per bone (click + to open) */}
                    ...
                    <div key={i} id={`neighbors-${slugify(String(child.name))}`} className="scroll-mt-24">
                      ...
                      <div className="rounded-xl border border-stone-200/80 bg-white p-4 sm:p-5">
                        <div className="grid gap-6 sm:grid-cols-[1fr_auto] sm:items-start">
                          <div className="min-w-0">
                            <div className="prose prose-sm max-w-none text-stone-700 prose-li:marker:text-stone-400" dangerouslySetInnerHTML={{ __html: String(child.description || "").replace(/\n/g, "<br/>") }} />
                            <p className="mt-3 text-xs text-stone-500">Click image to show/hide labels. Right click to enlarge.</p>
                          </div>
                          {child.views ? (
                            <div className="shrink-0">
                              <ImageViewer
      ...
      <section className={`rounded-2xl border px-6 py-6 sm:px-8 sm:py-7 ${STEP_STYLES.landmarks.bg} ${STEP_STYLES.landmarks.border}`}>
        ...
        {Array.isArray(landmarks) &&
          landmarks.map((bigPicture: Record<string, unknown>, idx: number) => {
            const landmarkElements = (bigPicture.elements as Record<string, unknown>[]) ?? [];
            const firstEl = landmarkElements[0] as Record<string, unknown> | undefined;
            return (
              <div key={idx} id={`landmarks-${slugify(String(bigPicture.big_picture_name))}`} className="mt-8 first:mt-6">
                <h3 className="font-serif text-lg font-semibold tracking-tight text-stone-900">{String(bigPicture.big_picture_name)}</h3>
                <div className="mt-3 grid gap-6 sm:grid-cols-[auto_1fr] sm:items-start">
                  ...
                  <div className="min-w-0">
                    ...
                    <ul className="list-disc pl-6 mb-2">
                      {landmarkElements.map((el: Record<string, unknown>, i: number) => {
                        ...
                        return (
                          <li key={i}>
                            <button
                              type="button"
                              onClick={() => openAndScrollToLandmark(detailAccordionId, targetId)}
                              className="text-aq-primary hover:underline text-left"
                            >
                              {isHtml(String(el.name)) ? renderHtml(String(el.name)) : String(el.name)}
                            </button>
                          </li>
                        );
                      })}
      ...
      <div className="rounded-2xl border border-stone-200/90 bg-stone-50/80 px-6 py-6 sm:px-8 sm:py-7">
        ...
        <ul className="list-disc pl-6 space-y-1 text-stone-700">
          <li><a href="https://www.kenhub.com/en/library/anatomy/the-humerus" target="_blank" rel="noopener noreferrer" className="font-medium text-aq-primary hover:underline">Kenhub – Humerus Anatomy</a></li>
          <li><a href="https://courses.lumenlearning.com/suny-ap1/chapter/bones-of-the-upper-limb/" target="_blank" rel="noopener noreferrer" className="font-medium text-aq-primary hover:underline">Lumen Learning – Bones of the Upper Limb</a></li>
        </ul>
        <h3 id="reference-images" className="text-lg font-medium mt-4 text-stone-800">Images</h3>
        <p className="text-stone-700">All images taken from <a href="https://www.z-anatomy.com/" target="_blank" rel="noopener noreferrer" className="font-medium text-aq-primary hover:underline">Z-Anatomy</a>. As of the date this was written (February 2, 2026), Z-Anatomy has a CC-BY-SA 4.0 License.</p>
      </div>
